...
<body class="bg-dark">
    ...
    $sql = "SELECT * FROM category_lists";
            ...
            <main class="col-10 ms-sm-auto col-lg-10 px-md-4 py-4">
                <nav aria-labels="breadcrumb">
                    <ol class="breadcrumb brdcrumb fs-4">
                        <li class="breadcrumb-item fw-bold"><a class="text-light" href="#">Categories</a></li>
                        <li class="breadcrumb-item" aria-current="page">Overview</li>
                    </ol>
                </nav>
                <div class="row mt-5">
                    <div class="card bg-light text-dark ms-2" style="width: 90% !important">
                        <h5 class="card-header text-center text-dark">Categories List of the News</h5>
                        <div class="card-body">
                            <a href="create.php" class="btn btn-block btn-success"><i class="fa-solid fa-plus me-3"></i>Add Category</a>
                                ...
                                <table class="table table-striped table-hover">
                                    ...
                                    <tbody>
                                        ...
                                        foreach ($datas as $key => $data) {
                                            $used = false;
                                            $count = $conn->query("SELECT * FROM news_details WHERE category_id =" . $data['category_id']);
                                            if ($count->num_rows > 0) {
                                                $used = true;
                                            }
                                            echo '
                                                <tr>
                                                    ...
                                                    <td class="text-center align-items-center">
                                                        <a href="show.php?id=' . $data['category_id'] . '" class="btn btn-sm btn-primary">View</a>
                                                    </td>
                                                    <td class="text-center align-items-center">
                                                        <a href="edit.php?id=' . $data['category_id'] . '" class="btn btn-sm btn-warning">Edit</a>
                                                    </td>
                                                    <td class="text-center align-items-center">' . ((!$used) ?
                                                '<a onclick="return confirm(`Confirm delete action?`)" href="delete.php?id=' . $data['category_id'] . '" class="btn btn-sm btn-danger">Delete</a>'
                                                :
