...
<body class="bg-dark">
    ...
    if (isset($_POST['submit'])) {
        ...
        if (!empty($_FILES['image']['name'])) //new image uploaded
        {
            ...
            $sql = "UPDATE news_details SET title ='$title', category_id = '$category', 
                                        date = '$date', image = '$image',
                                        paragraph = '$paragraph', creator_id = '$author', 
                                        status_id = '$status'
                WHERE news_id = $id;"; //save to DB with new image name
        } else // no image uploaded
        {
            ...
            $sql = "UPDATE news_details SET title ='$title', category_id = '$category', 
                            ...
                            paragraph = '$paragraph', creator_id = '$author', 
                            status_id = '$status'
                    WHERE news_id = $id;"; //save to DB but no change image column
        }
        $datas = $conn->query($sql);
            ...
            <main class="col-10 ms-sm-auto col-lg-10 px-md-4 py-4">
                <nav aria-labels="breadcrumb">
                    <ol class="breadcrumb brdcrumb fs-4">
                        ...
                        <li class="breadcrumb-item" aria-current="page">Edit News Details</li>
                    </ol>
                </nav>
                <div class="row mt-5">
                        ...
                        <div class="card-body">
                            <form action="<?php echo $_SERVER['PHP_SELF'] . "?id=" . $id ?>" method="POST">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="category" id="category" class="p-2 flex-fill btn opt" required>
                                        <?php foreach ($categories as $kategori) : ?>
                                            <option <?php echo ($category == $kategori['category_name']) ? 'selected' : '' ?> value="<?php echo ($kategori['category_id']) ?>" class="text-start">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="author" id="author" class="p-2 flex-fill btn opt" required>
                                        <?php foreach ($authors as $penulis) : ?>
                                            <option <?php echo ($author == $penulis['creator_name']) ? 'selected' : '' ?> class="text-start" value="<?php echo ($penulis['creator_id']) ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <select name="status" id="status" class="p-2 flex-fill btn opt" required>
                                        <?php foreach ($stats as $stat) : ?>
                                            <option <?php echo ($status == $stat['status_name']) ? 'selected' : '' ?> class="text-start" value="<?php echo ($stat['status_id']) ?>">
                                ...
                                <div class="mb-3">
                                    ...
                                    <input type="file" class="form-control" id="image" name="image" value="<?php echo $image ?>" accept="image/png, image/jpeg" selected>
                                </div>
                                <div class="mb-3">
